...
import random
# rule of the game:
# two players draw a random card from their deck once their deck is empty when try try to draw a card they automatically lose
# cards have attack value + defense value + name
# each round player choose one card to play once they both choosed the one with higher card power win this round
# there are three genere of card:
    # AI allow u to add top wtwo cards to ur deck via drawing
    # Tutor add a copy of the first card in ur hand but u will lose the current round as the cost
    # TA discard the card with the highest pwoer in ur hand and add the discarded card's attack and defense to this card
    # Instructor can survive multiple rounds as long sas it has a postive or zero power but it's stat will lose 1000 at the begining of each turn
# power = player'attack - player's defense
# the onw who first win 8 rounds wins the match
...
class Card:
    ...
    def __init__(self, name, attack, defense):
        ...
        assert isinstance(attack, int) and isinstance(defense, int)
        self.name = name
        self.attack =  attack
...
class AICard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        # BEGIN Problem 3
        "*** YOUR CODE HERE ***"
        player.hand += [player.deck.cards[0],player.deck.cards[1]]
...
class TutorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        if player.hand:
            ...
            added = True
        self.attack = opponent_card.attack - 1
        self.defense = opponent_card.defense -1       
        # END Problem 4
        # You should add your implementation above this.
        if added:
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        best_card = None
        if player.hand:
            best_card = [i for i in player.hand if i.power(opponent_card) == max(j.power(opponent_card) for j in player.hand)][0]
            self.attack += best_card.attack
            self.defense += best_card.defense
            player.hand = [i for i in player.hand if i != best_card]
        # END Problem 5
        if best_card:
...
class InstructorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        self.attack -= 1000
        self.defense -= 1000
        if self.defense >= 0 and self.attack >= 0:
            readd = True
            player.hand += [self]
        # END Problem 6
        # You should add your implementation above this.
