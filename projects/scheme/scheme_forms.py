...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        env.define(function_name, do_lambda_form(Pair(arguments, expressions.rest), env))
...
def do_and_form(expressions, env):
    ...
    while t.rest is not nil:
        if is_scheme_false(scheme_eval(t.first,env)):
...
def do_or_form(expressions, env):
    ...
    if t is nil:
        return False
    while t.rest is not nil:
        if is_scheme_true(scheme_eval(t.first,env)):
            return scheme_eval(t.first,env)
        t = t.rest
    return scheme_eval(t.first,env)
...
def do_cond_form(expressions, env):
    ...
    # return nothing is no predicate is true and no else identifer
    return None
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        t = bindings.first
        validate_form(t, 2, 2)
        names = Pair(t.first, names)
        vals = Pair(scheme_eval(t.rest.first, env), vals)
