...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            assert self.is_container != place.ant.is_container, f"Two ants in {place}"
            assert (self.is_container and self.can_contain(place.ant)) or (place.ant.is_container and place.ant.can_contain(self)), f"Two ants in {place}"
...
class FireAnt(Ant):
    ...
    implemented = True   # Change to True to view in the GUI
    # END Problem 5
    # only explictly modify health when ant's ininitial health is not 1
    ...
    def reduce_health(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        # for loops is essentially a iterator who accessing the newest list and return element by current index position
        # when modifying list and iterating the list use for i in self.place.bees[:] or for i in revresed(self.place.bees)
    ...
    def action(self, gamestate):
        if self.chew_timer:
            ...
            if self.place.bees:
                ...
                Insect.reduce_health(i, i.health)
...
class BodyguardAnt(ContainerAnt):
    ...
    def __init__(self, *args, **kwargs):
        self.ant_contained = None
        super().__init__(health = 2,*args, **kwargs)# Change to True to view in the GUI
    # END Problem 8
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if not gamestate.queenAntExist:
            gamestate.queenAntExist = True
    ...
    def action(self, gamestate):
        ...
        while place:
            # place.ant only connect with one ant object
            if place.ant and not place.ant.buffed_by_queen:
                place.ant.buffed_by_queen = True
                place.ant.damage *= 2
            # the contained ant still need to be buffed even though its container ant has been buffed
            if place.ant and place.ant.is_container and place.ant.ant_contained:
                tmp = place.ant.ant_contained
                if not tmp.buffed_by_queen:                        
                    tmp.buffed_by_queen = True
                    tmp.damage *= 2 
            place = place.exit
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        # it will not even attempt to move when slowed and time is odd
        # Extra credit: Special handling for bee direction
        # slowed
        if self.slowed_duration > 0:
            self.slowed_duration -= 1
            # doesnt attempt when bee is slowed and time is not even            
