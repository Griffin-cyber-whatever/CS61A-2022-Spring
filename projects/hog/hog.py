...
def roll_dice(num_rolls, dice=six_sided):
    ...
    for i in range(num_rolls):
        ...
        # dude why tf u have such weird configuration
        if s == 1:
            one_in_roll = True
        score += s
    if one_in_roll:
...
def hefty_hogs(player_score, opponent_score:int):
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        ...
        # BEGIN PROBLEM 7
        "*** YOUR CODE HERE ***"
        #we should expect both(say_scores, announce_lead_changes) as the given function argument 
        #dont need to specify it by ourselves
        leader ,message= say(score0, score1, leader)
        #we shouldnt expect that the first function for both is say_scores
        if message:
            print(message)
    # END PROBLEM 7
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0 > score1:
        ...
        if last_leader != leader:
            return leader, f"Player 0 takes the lead by {score0 - score1}"
        elif last_leader == leader:
            return leader, None  
    elif score0 < score1:
        leader = 1
        if last_leader != leader:
            return leader, f"Player 1 takes the lead by {score1 - score0}"
        elif last_leader == leader:
            return leader, None   
    return None, None
    # END PROBLEM 6
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if hog_pile(score, opponent_score):
        return 0
    hefty_hogs_score = hefty_hogs_strategy(score, opponent_score, threshold, num_rolls)
    if not hefty_hogs_score:
        return 0
    return num_rolls
    # Remove this line once implemented.
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    """Write a brief description of your final strategy.
        find the highest value of three following condition:
        highest value = score we get - score they might get from hog pile and hefty
        case 1:
            we just throwing dices + hog pile
            score we get = avg score per dice * throwing num + hog_pile(score, oppnoent_score)
            score they might get = hog_pile(opponent_score, score) + hefty(opponent_score, score)

        case 2;
            we choose hefty and hog pile
            score we get = hefty(score, opponent_score) + hog_pile(score, oppnoent_score)
            score they might get = hog_pile(opponent_score, score) + hefty(opponent_score, score)
        ...
        this can be further improved by caluculating further steps   
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    #what u gonna get for basically throwing dices
    
    #case 1
    num = max_scoring_num_rolls()
    avg = make_averaged(roll_dice, total_samples=1000)(num)
    get = num * avg
    case1_score = score + get
    case1_oppponent = hog_pile(opponent_score, case1_score) + hefty_hogs(opponent_score, case1_score)
    case1_value = case1_score - case1_oppponent
    
    #case 2
    get = hefty_hogs(score, opponent_score) + hog_pile(score, opponent_score)
    case2_score = score + get
    case2_oppponent = hog_pile(opponent_score, case2_score) + hefty_hogs(opponent_score, case2_score)
    case2_value = case2_score - case2_oppponent
