...
(define (closer-city lat lon city-a city-b)
  (let ((a (make-city 'temp lat lon))   ;; Create a new city 'a' with dummy name 'temp'
        (dist-a (distance (make-city 'temp lat lon) city-a))  ;; Distance to city-a
        (dist-b (distance (make-city 'temp lat lon) city-b))) ;; Distance to city-b
    (if (< dist-b dist-a) (get-name city-b) (get-name city-a))))  ;; Return closer city
...
(define (teacher-hold-class teacher)
    ...
    (teacher-create (teacher-get-name teacher) (teacher-get-class teacher) new-students)
)
