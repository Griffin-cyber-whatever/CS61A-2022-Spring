def reverse_link(lnk):
    ...
    """
    "*** YOUR CODE HERE ***"
    reverse_link = Link.empty
    while lnk is not Link.empty:
        reverse_link = (lnk.first, reverse_link)
        lnk = lnk.rest
    return reverse_link
...
def store_digits(n):
    ...
    while n > 0:
        l.append(n % 10)
        n //= 10
    # cant use str or reversed
    t = Link(l[len(l)-1])  #keep track on the first item of the list
    current = t   # keep track on the last item of the list
    for i in range(len(l) - 2, -1, -1):
        current.rest = Link(l[i])
